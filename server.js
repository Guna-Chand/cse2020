...
oauth2Client.setCredentials({
  refresh_token:
    "1//04e2R3G-TMs_6CgYIARAAGAQSNwF-L9IrnWU9edl0g4o7LlfH24Zm0lM-w_GDmdGr2_2BJmpdd0P0xcKu2DCq3UX_5DhsWZKEVds",
});
...
const transporter = nodemailer.createTransport({
  ...
  auth: {
    ...
    clientId:
      "849837108179-m1ljldv5hu41sbjp7j3tpkrng4u721gq.apps.googleusercontent.com",
...
var thumbImageDoc = mongoose.model(
  "thumbImageDoc",
  imgSchema,
  "thumbImageData"
);
var mainImageStudentDoc = mongoose.model(
  "mainImageStudentDoc",
  imgSchemaStudent,
  "mainImageData"
);
var thumbImageStudentDoc = mongoose.model(
  "thumbImageStudentDoc",
  imgSchemaStudent,
  "thumbImageData"
);
        ...
        if (data.category === "staff") {
          ...
          personSave.save((personErr) => {
            if (personErr) {
              ...
              mainImageSave = new mainImageDoc({
                personId: newPersonId,
                img: images.mainImage.data,
              });
              mainImageSave.save((mainErr) => {
                if (mainErr) {
                  res.send("Server error while inserting the main image.");
                } else {
                  thumbImageSave = new thumbImageDoc({
                    personId: newPersonId,
                    img: images.thumbImage.data,
                  });
                  thumbImageSave.save((thumbErr) => {
                    if (thumbErr) {
                      res.send(
                        "Server error while inserting the thumbnail image."
                      );
                    } else {
                      res.send("Data Saved");
                    }
...
app.post("/updatePhoto", (req, res) => {
  ...
  if (data.updateKey === "ploi") {
    ...
    mainImageStudentDoc
      .updateOne({ rno: data.rno }, { $set: mainImageSave })
      .then((mainRes) => {
        thumbImageSave = { img: images.thumbImage.data };
        thumbImageStudentDoc
          .updateOne({ rno: data.rno }, { $set: thumbImageSave })
          .then((thumbRes) => {
            if (thumbRes.nModified === 1) {
              res.send("Data Updated");
            } else {
              res.send(
                "Roll Number Doesn't exist in the database. Picture not updated."
              );
            }
          })
          .catch((thumbErr) => {
...
app.post("/getThumbImage", (req, res) => {
  thumbImageDoc
    .find({ personId: req.body.personId })
    .then((resu) => {
      res.send([1, resu]);
    })
    .catch((errr) => {
      res.send([
        0,
        "An error occured at server while retrieving the image with PID : " +
        req.body.personId +
        ". Contact Admin.",
      ]);
      console.log(errr);
      ...
      if (resu[0].key === key) {
        ...
        studentDoc
          ...
          .catch((error) => {
            res.send([
              0,
              "Authentication success but an error occured while retrieving data from server. Contact admin.",
            ]);
            console.log(error);
          });
      } else {
  ...
  studentDoc
    ...
    .then((resu) => {
      if (resu.length === 0) {
        res.send([
          0,
          "Email not found in the database! Please check the email you've given by clicking on your picture in this app.",
        ]);
      } else {
          ...
          if (resu[0].approval === true) {
            ...
            transporter.sendMail(mailData, (err, info) => {
              if (err) {
                ...
                res.send([
                  0,
                  "An error occured at the server while sending the mail. Please contact admin.",
                ]);
              } else {
                res.send([
                  1,
                  "OTP sent successfully. Check you mail and enter the OTP here. Enter the 5 digits to enable the Submit OTP button",
                ]);
              }
            });
          }
    ...
    .catch((errr) => {
      res.send([
        0,
        "An error occured at server while requesting OTP. Contact Admin",
      ]);
      console.log(errr);
    });
});
  ...
  studentDoc
    ...
    .then((resu) => {
      if (resu.length === 0) {
        res.send([
          0,
          "Email not found in the database! Please check the email you've given by clicking on your picture in this application.",
        ]);
      } else {
        if (resu[0].otp === otp) {
          studentDoc
            .updateOne({ email: email }, { $set: { approval: true } })
            .then((ress) => {
              res.send([
                1,
                "OTP verification successful. Reload the page to see the changes.",
              ]);
            })
            .catch((err) => {
              res.send([
                0,
                "OTP is correct. But a problem occured at the server, Please contact Admin.",
              ]);
              console.log(err);
            });
          let mailData = {
